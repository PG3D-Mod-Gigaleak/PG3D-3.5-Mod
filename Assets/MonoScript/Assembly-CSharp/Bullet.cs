...
public class Bullet : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (PlayerPrefs.GetInt("MultyPlayer") != 1 || (PlayerPrefs.GetInt("MultyPlayer") == 1 && ((PlayerPrefs.GetString("TypeConnect").Equals("local") && base.GetComponent<NetworkView>().isMine) || (PlayerPrefs.GetString("TypeConnect").Equals("inet") && photonView.isMine))))
		{
			base.transform.position += base.transform.forward * bulletSpeed * Time.deltaTime;
		}
		if (PlayerPrefs.GetInt("MultyPlayer") == 1 && ((PlayerPrefs.GetString("TypeConnect").Equals("local") && base.GetComponent<NetworkView>().isMine) || (PlayerPrefs.GetString("TypeConnect").Equals("inet") && photonView.isMine)) && GetDistance(startPos, base.transform.position) >= lifeS)
		{
